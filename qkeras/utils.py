...
def model_quantize(model,
  ...
  if transfer_weights:        
    for layer, qlayer in zip(model.layers, qmodel.layers):
      if layer.get_weights():
        tmp = layer.get_weights()[0:2]            #take layer's first two elements(kernel and bias) 
        tmp.extend(qlayer.get_weights()[2:])      #extend the list with the additional elements from qlayer
        qlayer.set_weights(copy.deepcopy(tmp))    #transfer weights
  return qmodel
