...
class quantized_bits_featuremap(BaseQuantizer):
  ...
  def __init__(self,
               ...
               use_ste=True,
               use_variables=False):
    super(quantized_bits_featuremap, self).__init__()
    self.bits = bits
    self.integer = integer
    self.symmetric = symmetric
    self.keep_negative = keep_negative
  ...
  def __call__(self, x):
    ...
    x = K.cast_to_floatx(x)
    mask = abs(x) < K.epsilon()
    x = tf.where(mask, tf.zeros_like(x), x) # epsilon correction due to mismatch with keras when inputs have to be zero but are not
    self.inputx = x
    ...
    if self.alpha is None:
      ...
      if self.alpha == "auto":
        ...
        beta_f = K.max(x, axis=axis, keepdims=True) 
        self.beta_f = beta_f 

        t = tf.math.divide_no_nan((beta_f*alphaq - alpha_f*betaq), (beta_f - alpha_f)) 
        mask = t > 0 
        zeropoint = tf.where(mask, tf.floor(t + 0.5), tf.math.ceil(t - 0.5)) 
        self.zeropoint = zeropoint 

        u = tf.math.divide_no_nan(x, scale) + zeropoint 
        #u = u - tf.sign(u) * K.epsilon() # epsilon correction due to mismatch with keras, ideal
        u = u - 0.00001526 # epsilon correction due to mismatch with keras, only minus to simplify hardware implementation
        self.before_rounding = u 
        mask = u > 0 
        v = tf.where(mask, tf.floor(u + 0.5), tf.math.ceil(u - 0.5)) 
        z = tf.clip_by_value(v, alphaq, betaq) 
        self.outputq = z 

        xq = scale * (z - zeropoint) 
