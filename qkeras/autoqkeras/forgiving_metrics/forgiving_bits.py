...
class ForgivingFactorBits(ForgivingFactor):
  ...
  def _param_size(self, layer):
    ...
    parameter_size = 0
    t_size_bias = 31 #marco (this is the reference for the bias, otherwise it would be equal to t_size just like the weights)
    ...
    if layer.__class__.__name__ in [
      ...
      #for w in layer.get_weights():                       #marco
      for i,w in enumerate(layer.get_weights()):           #add enumerate to distinguish kernel from bias
        #print("INDEX,WSHAPE",i,w.shape)
        if (i == 1):                                        #marco 
          parameter_size += t_size_bias * np.prod(w.shape)  #if bias use the bias reference size
        else:
          parameter_size += t_size * np.prod(w.shape)
    elif layer.__class__.__name__ in [
        "QDense", "QConv2D", "QConv1D", "QDepthwiseConv2D","QConv2DBatchnorm","QDepthwiseConv2DBatchnorm"]: #marco, added folded layers: QConv2DBatchnorm and QDepthwiseConv2DBatchnorm
      for i, w in enumerate(layer.get_weights()[0:2]):                                                      #marco, added 0:2 because folded layers have more than 2 elements
      ...
      # scale
      index = -1
      #parameter_size += t_size * np.prod(layer.get_weights()[index].shape) #marco
      # center (bias)
      if layer.center:
      ...
      index = -1
      bits = 6
      #parameter_size += bits * np.prod(layer.get_weights()[index].shape)   #marco
      # center (bias)
      if layer.center:
        bits = 5
        index = int(bool(layer.scale))
  ...
  def _act_size(self, layer):
    ...
    if layer.__class__.__name__ in ["InputLayer"]:
      ...
      if is_linear:
        bits = 0
      elif is_softmax:  #marco, removed: or is_sigmoid (I only use sigmoid as a fake layer)
        bits = o_size
      else:
        if isinstance(layer.activation, six.string_types):
